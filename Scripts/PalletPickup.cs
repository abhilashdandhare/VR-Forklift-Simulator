...
public class PalletPickup : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            transform.position = attachedToFork.position + attachedToFork.TransformDirection(localAttachOffset);
    ...
    private void AttachToFork(Transform forkTransform)
    {
        ...
        // Calculate local offset from fork
        localAttachOffset = forkTransform.InverseTransformDirection(transform.position - forkTransform.position);
        ...
        Debug.Log($"[Pallet] {gameObject.name} picked up!");
    }
